...
# ==================== CONFIGURATION ====================
BOT_TOKEN = "8586468507:AAHE6NP26myBwLn8I94F0XyHUAwITSmRdmQ"
...
def is_duplicate(scheduled_signals, new_sig):
    for sid, data in scheduled_signals.items():
        sig = data['signal']
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d.startswith('w_') or d.startswith('l_') or d.startswith('mw_') or d.startswith('a_'):
        ...
        ch = info.get('channel', DEFAULT_CHANNEL)
...
async def msg(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'OTC' in text.upper() or '☞' in text:
        ...
        for sig in parsed:
            if is_duplicate(ud['scheduled_signals'], sig):
                skipped += 1; continue
